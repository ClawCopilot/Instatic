...
async function handleAiChat(
  ...
  const { conversationId, content, snapshot, skillIds } = chatBody
  ...
  const textContent = content.find((b: AiContentBlock) => b.kind === 'text')
  const promptText = textContent && 'text' in textContent ? textContent.text : ''
  const autoIds = recommendSkills(promptText)
